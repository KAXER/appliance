...
void uploadIpScript()
{
  File script = FileSystem.open("/tmp/getip.sh", FILE_WRITE);
  script.print("#!/bin/sh\n");
  script.print("ifconfig | grep -o inet\\ addr:192.[0-9]*.[0-9]*.[0-9]* | grep -o 192.[0-9]*.[0-9]*.[0-9]*");
  script.close();
  ...
  chmod.addParameter("/tmp/getip.sh");  // path to the file to make it executable
  chmod.run();
}
